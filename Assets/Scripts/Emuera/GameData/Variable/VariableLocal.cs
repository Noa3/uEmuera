	...
	internal sealed class VariableLocal
	{
        ...
        public LocalVariableToken GetNewLocalVariableToken(string subKey, FunctionLabelLine func)
        {
			...
			else
			{
                ...
                {
                    if (!func.IsSystem)
						ParserMediator.Warn("function宣言にargumentvariable\"" + varCode + "\"が使われていnotfunctioninsideで\"" + varCode + "\"が使われています(functionのargument以outのfor途に使うthisは推奨されません.代わりに#DIMのuseを検討してください)", line, 1, false, false);
                    else
						ParserMediator.Warn("systemfunction" + func.LabelName + "insideで\"" + varCode + "\"が使われています(functionのargument以outのfor途に使うthisは推奨されません.代わりに#DIMのuseを検討してください)", line, 1, false, false);
                }
				//throw new CodeEE("このfunctionにargumentvariable\"" + varCode + "\"はdefinitionされていません");
			}
